...
class PDFAdapter:
  ...
  ##########
  def convert_bounding_poly(
    bounding_poly, page_width: float, page_height: float
  ) -> BoundingBox:
  ...
  def extract_text_from_text_anchor(text_anchor, document_text: str) -> str:
    ...
    for segment in text_anchor.text_segments:
      ...
      if 0 <= start_index < end_index <= len(document_text):
        ...
        logger.warning(
          f"Text segment indices [{start_index, end_index}] are out of bounds. Getting a partial string"
        )
        # There are a couple of options:
        # 1. Handle error: Here we log it and try to get at least a partial string.
        # 2. You could also just skip it
        extracted_text += document_text[
          start_index : min(end_index, len(document_text))
        ]
  ...
  def google_doc_to_data(
    ...
    if use_cache:
      if os.path.isfile(cache_path):
        logger.info(
          f"Cached JSON file FOUND at '{cache_path}'. Loading it as document!"
        )
        doc = load_document_from_json(cache_path)
      else:
        logger.info(
          f"Cached JSON file NOT FOUND at '{cache_path}'. Fallback activated, querying Google OCR!"
        )
    ...
    for index, page in enumerate(doc.pages):
      ...
      for token in page.tokens:
        ...
        token_text = PDFAdapter.extract_text_from_text_anchor(
          token.layout.text_anchor, doc.text
        )
        if not token_text.strip():
          continue
        parsed_token = Token(
          text=token_text,
          bbox=PDFAdapter.convert_bounding_poly(
            token.layout.bounding_poly,
            pdf_doc[index].rect.width,
            pdf_doc[index].rect.height,
          ),
          ...
          text_segments=[
            TextSegment(start_index=t.start_index, end_index=t.end_index)
            for t in token.layout.text_anchor.text_segments
          ],
        )
        tokens_per_page.append(parsed_token)

      # Create PageData object for the current page and add it to our pages list
      page_data = PageData(tokens=tokens_per_page)
      pages.append(page_data)

    logger.info(
      "Successfully parsed Google DocAI API Response, returning document data!"
    )
  ...
  def fitz_to_data(doc: fitz.Document) -> DocumentData:
    ...
    # Start and ending pointers, these persist across loop iterations
    start = 0
    end = 0
    for page_num, page in enumerate(doc):
      ...
      for token_info in tokens:
        ...
        if (
          token_text.strip() == ""
        ):  # Skip any words/tokens that don't have meaningful text.
          continue

        end = start + len(token_text)
        segment = TextSegment(start_index=start, end_index=end)
      ...
      page_data_list.append(PageData(tokens=parsed_tokens))

    # Remove that last unnecessary whitespace character; memory efficient
